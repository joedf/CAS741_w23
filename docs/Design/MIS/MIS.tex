...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\code{updateInfo} & \code{textDisplayControl}, \code{baseImage}, & - & \code{DifferentImageSizes} \\
...
\item exception: \code{DifferentImageSizes} meaning the size of \code{baseImage} and
  \code{resultImage} do not match.
\end{itemize}
...
\subsubsection{State Variables}
References to drawing stages/canvases for all the visualization/display modules mentioned
in section \ref{USES_dispControl}, along with each corresponding update function references.
...
\noindent \code{drawGrid(gridLayer, rect, rows, cols, lineColor)}:
\begin{itemize}
...
\begin{itemize}
\item transition: Draw a given geometry (\code{shape}) or image object repeated over a grid pattern
...
\mref{M_metric} Image Metrics Calculation
