...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\code{drawResultingSubregion} & \code{stage}, \code{subregionImage},
...
\begin{itemize}
  \item \code{stage}: a reference to the drawing stage.
  \item \code{subregionImage}: a reference to the subregion image.
  \item \code{gtImage}: a reference to the ground truth image.
  \item \code{imgParams}: a reference to function to call to get the rasterization
    grid parameters (number of rows \code{imageRows} and columns \code{imageCols},
    and magnification (\code{imageMag}) 
    provided by \mref{M_params}
...
\noindent \code{drawResultingSubregion(stage, subregionImage, gtImage, imgParams, beam)}:
\begin{itemize}
\item transition: On the given stage, draws the resampled \code{subregionImage}
  ...
  The \code{gtImage} is used as the actual image data source instead of the
  \code{subregionImage} for visual fidelity, but using the cropped region defined
  by the \code{subregionImage}.
\item output: a reference to the update function (\code{doUpdate}).
\end{itemize}
