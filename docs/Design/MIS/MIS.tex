...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\code{drawGroundtruthImage} & \code{stage}, \code{gtImage}, \code{subregionImage}, \code{boxSize}
...
\noindent \code{drawGroundtruthImage(stage, gtImage, subregionImage, boxSize)}:
\begin{itemize}
...
\subsubsection{Local Functions}
\code{doUpdate()}: Defines a drawing arrangement to fill the stage with a image bounded by \code{rect}.
