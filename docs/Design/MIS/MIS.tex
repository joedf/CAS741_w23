...
tuples, and \code{imageDrawingObject}. Sequences are lists filled with elements of the same data type. Strings
are sequences of characters. Tuples contain a list of values, potentially of
different types. \code{imageDrawingObject} is a geometry object (provided by Konva):
...
\subsubsection{Environment Variables}
\begin{itemize}
  \item Display/Canvas controls as provided by the GUI \mreff{M_GUI}.
  \item Mouse
  \item Keyboard
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\code{drawGrid} & \code{gridLayer}, \code{rect},
  \code{rows} ($\mathbb{Z}_+$), \code{cols} ($\mathbb{Z}_+$), \code{lineColor}
  & cell size (width/height in pixels, $\mathbb{Z}_+$) & \code{badGridParams} \\
\hline
\code{repeatDrawOnGrid} & \code{layer}, \code{rect}, \code{rows} ($\mathbb{Z}_+$),
  \code{cols} ($\mathbb{Z}_+$), \code{shape}
  & - & \code{badGridParams} \\
\hline
...
\subsubsection{Environment Variables}
The graphics processing hardware. \an{or optional software rendering?}
...
\code{drawResultingSubregion} & \code{stage}, \code{subregionRect},
  \code{gtImage}, \code{imgParams}, \code{beam} & \code{doUpdate} & - \\
\hline
\end{tabular}
\end{center}
