...
\subsubsection{Local Functions}
\begin{itemize}
  \item \code{doUpdate()}: a local copy (of the function described above) to call for GUI events such as mouse clicks.
  \item \code{updateInfoDisplay()} and \code{doExport()}: as described above.
\end{itemize}
...
\begin{itemize}
\item transition: Defines (in a function \code{doUpdate}) a drawing arrangement to fill the stage with the ground truth image
