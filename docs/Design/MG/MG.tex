...
\begin{description}
\item[Secrets:] \an{The format and structure of the input data.}
\item[Services:] \an{Converts the input data into the data structure used by the
...
\subsubsection{Image Compositing (\mref{M_compositing})}
\begin{description}
\item[Secrets:]\an{The format and structure of the input data.}
\item[Services:]\an{Converts the input data into the data structure used by the
  input parameters module.}
\item[Implemented By:] CanvasAPI (globalCompositeOperation or CanvasRenderingContext2D.clip())
\item[Type of Module:] \an{[Record, Library, Abstract Object, or Abstract Data Type]
  [Information to include for leaf modules in the decomposition by secrets tree.]}
\end{description}
...
\item[Services:]\an{Converts the input data into the data structure used by the
  input parameters module.}
\item[Implemented By:] Konva and ImgBeamer
\item[Type of Module:] \an{[Record, Library, Abstract Object, or Abstract Data Type]
  [Information to include for leaf modules in the decomposition by secrets tree.]}
\end{description}


\subsubsection{Image Sampling (\mref{M_sampling})}
\begin{description}
\item[Secrets:]\an{The format and structure of the input data.}
\item[Services:]\an{Converts the input data into the data structure used by the
  input parameters module.}
\item[Implemented By:] CanvasAPI and ImgBeamer
\item[Type of Module:] \an{[Record, Library, Abstract Object, or Abstract Data Type]
  [Information to include for leaf modules in the decomposition by secrets tree.]}
\end{description}
