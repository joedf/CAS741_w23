...
2023/03/26 & 1.2.2 & Fill in expected outputs and justifications for
  the unit tests in section \ref{sec_func_req_unittest}. \\
\bottomrule
\end{tabularx}
...
\begin{table}[H]
  ...
  \midrule
    \code{original\_500-crop.png} & \code{og} & a virtual image representing metallic grains. \\
    \code{og-fant.png} & \code{fant} & the original image resampled using
      Fant's algorithm \cite{fant_1986} as 50x50 pixel image 
      and scaled back up using nearest neighbor. \\
    \code{black.png} & \code{blak} & all black (\code{0x00} or \code{0}) pixels. \\
    \code{white.png} & \code{wite} & all white (\code{0xFF} or \code{255}) pixels. \\
    \code{half.png} & \code{half} & half white, half black pixels. \\
    \code{gray.png} & \code{gray} & all pixels are gray (\code{0x80} or \code{128},
      half of the ``colour range'' of the 8 bit depth). \\
    \code{c10-010-010.png} & \code{c010} & 10x pixel size, sampled with a spot of 10\% by 10\% \\
    \code{c10-060-060.png} & \code{c060} & 10x pixel size, sampled with a spot of 60\% by 60\% \\
    \code{c10-100-100.png} & \code{c100} & 10x pixel size, sampled with a spot of 100\% by 100\% \\
    \code{c10-120-120.png} & \code{c120} & 10x pixel size, sampled with a spot of 120\% by 120\% \\
    \code{c10-130-130.png} & \code{c130} & 10x pixel size, sampled with a spot of 130\% by 130\% \\
    \code{c10-500-500.png} & \code{c500} & 10x pixel size, sampled with a spot of 500\% by 500\% \\
    \code{c10-060-500.png} & \code{c6a5} & 10x pixel size, sampled with a spot of 60 and 500\% at \ang{45}\\
    \code{og-minor\_defects.png} & \code{mDef} & the original image manually
...
Input: \code{black.png} and \code{white.png} (see \ref{Tbl_TestImages})

Output: \code{0.00} $\pm$ ERR\_MARGIN

Test Case Derivation: Should the lowest score/match possible
...
Input: \code{black.png} and \code{half.png} (see \ref{Tbl_TestImages})
...
Input: \code{white.png} and \code{half.png} (see \ref{Tbl_TestImages})
...
Input: \code{black.png} and \code{gray.png} (see \ref{Tbl_TestImages})
...
Input: \code{white.png} and \code{gray.png} (see \ref{Tbl_TestImages})
...
\item{\textbf{U\refstepcounter{unittestnum}\theunittestnum \label{U_metric6}}: control - all gray vs. half black and white}

Type: Automatic

Input: \code{gray.png} and \code{half.png} (see \ref{Tbl_TestImages})

Output: \code{0.50} $\pm$ ERR\_MARGIN

Test Case Derivation: One half of the pixel follow the justification of \utref{U_metric4},
...
\item{\textbf{U\refstepcounter{unittestnum}\theunittestnum \label{U_metric7}}: Lowest/Highest MSE, RMSE and NRMSE scores}

Type: Automatic

Input/Condition: The ground truth images \an{Prof says What image?}
...
How test will be performed: Executing a suite of tests \an{Prof says What tests?}
  for the multiple images.

\item{\textbf{U\refstepcounter{unittestnum}\theunittestnum \label{U_metric8}}: Control testing}
