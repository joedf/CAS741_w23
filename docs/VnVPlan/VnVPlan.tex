...
Output: \code{1.00} $\pm$ ERR\_MARGIN (or 100\% relative)

Test Case Derivation: Should be the best score/match possible
...
Output: \code{0.99} $\pm$ ERR\_MARGIN (or 99\% relative)

Test Case Derivation: Should be very close to the best score/match possible
...
Test Case Derivation: Should be the worst score/match possible
...
Output: \code{0.29} $\pm$ ERR\_MARGIN (or 29\% relative)
...
Output: \code{0.50} $\pm$ ERR\_MARGIN (or 50\% relative)
...
python package (which implements MSE, RMSE, PSNR, UIQ/UQI, SSIM, MS-SSIM, SCC, VIF, and more)
...
\begin{itemize}
  ...
  \item \href{https://github.com/darosh/image-ms-ssim-js}{image-ms-ssim.js} (\textbf{L\refstepcounter{libnum}\thelibnum \label{lib_msssim}})
    -- implements MS-SSIM and SSIM (by ``darosh'')
\end{itemize}
...
Output/Result: The expected order (from worst to best) of the metric scores is
$
\code{c500} \le \code{c6a5} \le \code{c010} \le \code{c060} \le \code{c100} \le
\code{c130} \le \code{c120}
$.
If we include the ground truth test images, this continues with
$
... \le \code{fant} \le \code{mDef} \le \code{og}
$
...
\begin{itemize}
  ...
  \item \code{c130} should score slightly better as there is more coverage, but may spread 
    the error more
  \item \code{c120} should score the highest/best as reasonable compromise between area coverage
    and error spread
  \item \code{fant} should score even better as it uses a complex high quality
    algorithm \cite{fant} must is still downsampled for testing.
  \item \code{mDef} should get an almost perfect score as it is not downsampled and only
    a few pixels that are different.
  \item \code{og} should be a perfect score as it is the exact same image as the original
    ground truth image.
\end{itemize}
...
\begin{enumerate}
  ...
  \item The \code{fant} ground truth image is used as the "ideal scenario" for a
    ...
    being significantly better/higher overall.
...
\begin{itemize}
  ...
  \item \utref{U_metric2} Test Variant: a worse score than the base variant,
  ...
  \item \utref{U_metric6} Test Variant: a worse score than the base variant,
    since ``structural similarity'' is taken into account.
\end{itemize}
