...
Accept an input image in the following formats:
- PNG (Portable Net Graphics)
– JPG (Joint Photographic Experts Group)
– BMP (Bitmap)
14
– preloaded example images
R2: Accept user input of the spot profile (size and shape) and ensure that all its defining
values are positive real numbers.
R3: Accept user input of pixel size and ensure it is a positive real number.
R4: Accept user input to specify a subregion (FOV) for processing.
R5: Display a representative spot layout used for processing the image.
R6: Reprocess (using IM1) and display the resulting image to the user.
R7: Calculate and display an image quality metric (according to IM2) to the user.
